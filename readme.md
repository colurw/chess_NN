...
1000x faster than Stockfish can). In the remaining 37% of cases, it plays a legal move of unknown quality.
...
contain any openings (the first 10 or so moves by each player).
...
Originally, in restricted/idealised test conditions 24% of its moves were random 
picks from a list of legal moves generated by the Python-Chess library.  This occurred 
when all the stages of the ensemble's voting criteria have been exhausted without 
producing a sensible result.  In real gameplay, this means that the model would likely play a 
streak of poor random moves, which could be taken advantage of by a 
...
Note, it may be possible to extract much more (3-5x) training data from 
the Lichess.com puzzle sequences (The encoder currently only uses the first step of each sequence).
Also, given extra resources, larger neural networks could be trained.  Making both
...
Python Chess library, and the legal move with the highest cosine similarity to the ensemble's raw average prediction is selected. 
