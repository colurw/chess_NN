...
Experimenting with adding noise during solving, then taking the average of all solutions
led to training several models, each on a portion of the training data.  By combining
the resulting predictions in various ways, we can avoid many instances of the two most
common failure modes: a piece being cloned, or a piece disappearing,
during a move.
...
streak of poor random moves in a row, which can easily be taken advantage of by a 
