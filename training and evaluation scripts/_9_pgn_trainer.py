...
PATIENCE = 3
BATCH_SIZE = 512
NOTES = f'n=1024,bn,1024_bs={BATCH_SIZE}_{str(DATA_LIMIT)[0:4]}k_wg1'
...
# Reshape y data to suit network with 64 branched outputs: list[square, ndarray[puzzle, piece_ohe_vector]]
yy_train = reshape_output(y_train, 64)
yy_test = reshape_output(y_test, 64)

# Map files to storage
...
fig.suptitle('gen_solv: '+NOTES+'  '+str(minutes)+'_min'+'  '+score+'%_solved')
...
plt.savefig(r'training graphs/general/{}.png'.format(NOTES))
