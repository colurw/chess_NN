...
all_puzzles = []
all_solutions = []
with open('training data/allPuzzles_w.csv', 'r') as csvfile:
    ...
    for row in reader:
        ...
        # Get moves
        moves = str(row[2]).replace('q', '')    # 'e5f6q e8e1 g1f2 e1f1' -> 'e5f6 e8e1 g1f2 e1f1'
        ...
        arr_flipped = np.flipud(arr_flipped)    # omit this line when _conv_w.csv loaded
...
with open("training data/all_puzz_w.pickle", "wb") as file:
    pickle.dump((all_puzzles), file)

with open("training data/all_solns_w.pickle", "wb") as file:
